...
export class TypeOrmUserRepository implements IUserRepository {
  ...
  async findByEmail(email: string): Promise<User | null> {
    ...
    return new User(ormUser.id, ormUser.name, ormUser.email, ormUser.role as UserRole, ormUser.isActive);
  }
  ...
  async findAll(): Promise<User[]> {
    ...
    return users.map(
      (ormUser) =>
        new User(ormUser.id, ormUser.name, ormUser.email, ormUser.role as UserRole, ormUser.isActive),
    );
  }
}